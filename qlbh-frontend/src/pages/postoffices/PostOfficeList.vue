<template>
  <div class="container mt-4">
    ...
    <div class="search-filter mb-3 d-flex align-items-center gap-2">
      <div class="d-flex gap-2 align-items-center flex-nowrap" style="flex: 1 1 auto;">
        <input
          ...
          placeholder="Tìm kiếm bưu cục theo tên chi nhánh, hotline, ID..."
          style="min-width: 300px; flex: 1 1 400px;"
        >
        <button
          @click="reloadPostOffices"
    ...
    <div v-if="postOffices.length === 0 && searchQuery && !loading" class="alert alert-info shadow-sm">
    ...
    >
      <table class="table table-bordered table-hover align-middle mb-0">
        ...
        <tbody>
          ...
          <!-- DATA -->
          <template v-else-if="paginatedPostOffices.length > 0">
            <tr v-for="p in paginatedPostOffices" :key="p.id">
              ...
              <td>
                <small class="text-muted">{{ p.hotline }}</small>
              </td>
              
              <td>
                <div class="d-flex justify-content-center gap-2">
                  <router-link :to="`/admin/postoffices/edit/${p.id}`" class="btn btn-light btn-sm border shadow-sm px-2">
                    Sửa
                  </router-link>
...
const paginatedPostOffices = computed(() => {
  ...
  return postOffices.value.slice(start, end)
})
